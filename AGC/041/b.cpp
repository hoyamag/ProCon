...
LL ProbS(int jj, LL N, LL M, LL V, LL P, VEC<LL> &A) {
  // return value
  //DUMP("ProbS", jj, N, M, V, P, A);
  VEC<LL> B = A;
  B.erase(B.begin() + jj);
  // //DUMP("ProbS, B", B);

  VEC<LL> C = VEC<LL>(B.begin() + P - 1, B.end());
  return ProbT(N - P, M, V - P, C);
}
